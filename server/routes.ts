...
export async function registerRoutes(app: Express) {
  ...
  // API endpoint for Global Image Editor - List all server images with directory structure  
  app.get("/api/images", (req, res) => {
    try {
      ...
      const getImagesFromDirectory = (dirPath: string, relativePath: string = ''): any[] => {
        const items: any[] = [];
        ...
        files.forEach(file => {
          ...
          if (stats.isDirectory() && file !== 'IG') {
            // Recursively get images from subdirectories (except IG)
            const subDirImages = getImagesFromDirectory(fullPath, path.join(relativePath, file));
            items.push(...subDirImages);
      ...
      const allImages = getImagesFromDirectory(imagesPath);
      
      // Sort by directory, then by filename
      allImages.sort((a, b) => {
        if (a.directory !== b.directory) {
          return a.directory.localeCompare(b.directory);
        }
  ...
  // API endpoint for Global Image Editor - List uploaded images
  app.get("/api/uploaded-images", (req, res) => {
    try {
      ...
      const files = fs.readdirSync(uploadsPath).filter(file => {
        ...
        const stats = fs.statSync(filePath);
        return !stats.isDirectory() && /\.(jpg|jpeg|png|gif|webp)$/i.test(file) && file.startsWith('upload_');
      });
